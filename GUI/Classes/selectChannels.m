function varargout = selectChannels(varargin)
...
%      applied to the GUI before selectChannels_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to selectChannels_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 16-May-2018 09:14:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @selectChannels_OpeningFcn, ...
                   'gui_OutputFcn',  @selectChannels_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function selectChannels_OpeningFcn(hObject, eventdata, handles, varargin)
...
if nargin == 4 && isstruct(varargin{1})
    names = varargin{1}.names;
    wavs = varargin{1}.wav_names;
    wvcs = varargin{1}.wvcs;
    types = varargin{1}.type_names;
    wav_values = varargin{1}.w_values;
    t_values = varargin{1}.t_values;
    record = varargin{1}.recording;
elseif nargin>=8
    names = varargin{1};
    wavs = varargin{2};
    wvcs = varargin{3};
...
for index = 1:length(record)
    get_from(record(index)) = 1;
end
...
if nargin > 3 && (isstruct(varargin{1}) || nargin>=8)
...
function [varargout] = selectChannels_OutputFcn(hObject, eventdata, handles) 
...
for index = 1:length(handles.channels)
    ...
    IDs{index} = {wavs{index}, wvcs(index)};
    ...
    if get(handles.channels(index).rec, 'Value')
        record(length(record)+1) = index;
    end
end
