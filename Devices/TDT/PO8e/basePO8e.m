    ...
    methods
        function obj = basePO8e()
            ...
            try
                ...
                disp('error loading library, make sure one of the following compilers is installed:\n')
                mex -v -setup
                return
            end
            if ~isempty(warnings)
        ...
        function [n_samples, obj] = samplesReady(obj, index)
            [n_samples, p, obj.StoppedPointer{index}] = calllib(obj.LibName, 'samplesReady', obj.CardPointer{index}, obj.StoppedPointer{index});
        end
